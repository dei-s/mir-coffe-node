...
class SponsorshipDiffTest extends PropSpec with PropertyChecks with Matchers with TransactionGen {
    ...
    forAll(setup) {
      case (genesis, issue, sponsor, assetOverspend, insufficientFee, coffeOverspend) =>
        ...
        assertDiffEi(setupBlocks, block(Seq(insufficientFee)), s) { blockDiffEi =>
          blockDiffEi should produce("does not exceed minimal value of 100000 COFFE")
        }
  ...
  property("cannot cancel sponsorship") {
    ...
    forAll(setup) {
      case (genesis, issueTx, sponsorTx, senderNotIssuer, insufficientFee) =>
        ...
        assertDiffEi(setupBlocks, block(Seq(insufficientFee)), s) { blockDiffEi =>
          blockDiffEi should produce("does not exceed minimal value of 100000000 COFFE: 99999999")
        }
    }
  }
  ...
  property("sponsor has no COFFE but receives them just in time") {
    val s = settings(0)
    val setup = for {
      master    <- accountGen
      recipient <- accountGen
